...
Future<void> _loadUserPosts() async {
  try {
    ...
    if (mounted) {
      ...
      print('✅ Loaded ${allUserPosts.length} posts for user ${widget.userId} (Role: $userRole)');
      if (userRole?.toLowerCase() == 'instructor' || userRole?.toLowerCase() == 'ta') {
        print('   ℹ️ Instructor/TA: Only showing regular posts');
      } else {
        print('   - ${regularPosts.length} regular posts');
        print('   - ${allUserPosts.where((p) => p['post_type'] == 'announcement').length} announcements');
        print('   - ${allUserPosts.where((p) => p['post_type'] == 'competition_request').length} competition requests');
      }
    }
  } catch (e) {
                        ...
                        child: Container(
                          ...
                          child: Row(
                            children: const [
                              ...
                              Icon(Icons.open_in_new, size: 18, color: Colors.blueGrey),
                            ],
                          ),
                        ),
                      ),
